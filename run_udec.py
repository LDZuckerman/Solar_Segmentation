...
'''
Script to pre-train a unet decoder to predict photometry from an algorithmic segmentation
'''

def run_udec(d, gpu, test_only=False):
    
    # Set out and data dirs
    net_name = d['net_name'] 
    outdir = "../model_runs_dec" # create this before running
    segdir = d['seg_dir'] # This is now X, not y (passing seg_as_input to dataloader)
    imgdir = d['img_dir'] # This is now y not X (passing seg_as_input to dataloader)
    ...
    train_ds = data_utils.dataset(image_dir=imgdir, mask_dir=segdir, set='train', norm=False, n_classes=d['n_classes'], channels=channels, randomSharp=d['randomSharp'], im_size=im_size, seg_as_input=True) # multichannel=True, channels=['deltaBinImg'], 
    train_loader = DataLoader(train_ds, batch_size=d['batch_size'], pin_memory=True, shuffle=True)
    test_ds = data_utils.dataset(image_dir=imgdir, mask_dir=segdir, set='val', norm=False, n_classes=d['n_classes'], channels=channels, randomSharp=d['randomSharp'], im_size=im_size, seg_as_input=True) # multichannel=True, channels=['deltaBinImg'],
    ...
    #ch_mul = 64 #run_utils.get_ch_mul(d['depth'], im_size)
    if im_size/2**d['depth'] < 1:
        raise ValueError(f'Depth is too high for im_size {im_size}')
    model = models.UDec(n_classes=d['n_classes'], out_chans=out_channels, depth=d['depth'], double=d['double'], ch_mul=64, kernel_size=d['kernel_size'], padding_mode=d['padding_mode'], reconstruct_mag=d['reconstruct_mag']).to(device)
    loss_func = run_utils.get_loss_func('multichannel_MSE_loss') 
    ...
    if not eval(str(test_only)):
        ...
        for epoch in range(d['num_epochs']):
            ...
            loss = run_utils.train_net(train_loader, model, loss_func, loss_name='multichannel_MSE_loss', optimizer=optimizer, chan_weights=d['weights'], device=device, save_examples=save_examples, save_dir=exp_outdir, epoch=epoch)
    ...
    model = models.UDec(n_classes=d['n_classes'], out_chans=out_channels, depth=d['depth'], double=eval(str(d['double'])), ch_mul=64, kernel_size=d['kernel_size'], padding_mode=d['padding_mode'], reconstruct_mag=eval(str(d['reconstruct_mag']))).to(device)
